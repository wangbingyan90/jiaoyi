...
class ClientTrader(IClientTrader):
    ...
    def cancel_entrust(self, entrust_no):
        self.refresh()
        for i, entrust in enumerate(self._get_grid_data(self._config.COMMON_GRID_CONTROL_ID)):
    ...
    @perf_clock
    def have_entrust(self,result,tig = 0):
        num = 0
        self._app.top_window().type_keys('{F5}')
        time.sleep(0.2)
        data = self._get_grid_data(self._config.COMMON_GRID_CONTROL_ID)
        for i, entrust in enumerate(data):
            if entrust['合同编号'] == result['卖出合同']:
                num = num + 1
                ht = result['买入合同']
                nums = i
            if entrust['合同编号'] == result['买入合同']:
                num = num + 2
                ht = result['卖出合同']
                nums = i
        if num == 3:
            return {'state':'平'}
        if num == 2:
            return {'state':'涨','未完成合同':ht,'nums':nums+1}
        if num == 1:
            return {'state':'跌','未完成合同':ht,'nums':nums+1}
        if num == 0:
            return {'state':'震'}


    def closew(self):
        self._app.window_(found_index=0).close()
    ...
    @perf_clock
    def aout_buy_sell(self, security, buyprice, sellprice,amount, **kwargs):
        self._switch_left_menus(["双向委托[F6]"])
        self._type_edit_control_keys(self._config.TRADE_SECURITY_CONTROL_ID, security)
        self._type_edit_control_keys(1035, security)
        self._type_edit_control_keys(1034, amount)
        self._type_edit_control_keys(1039, amount)
        time.sleep(0.1)
        ...
        cost = buyprice
        income = sellprice
        self._type_edit_control_keys(1033, cost)
        self._type_edit_control_keys(1058, income)
        # embed()

        self._main.child_window(
            control_id=1007, class_name="Button"
        ).click()
    ...
    def _set_trade_params(self, security, price, amount):
        ...
        price = float(self.main.child_window(control_id=1024, class_name="Static").window_text())
    ...
    def _get_left_menus_handle(self):
        ...
        while True:
            ...
            # pylint: disable=broad-except
                ...
                print('error occurred when trying to get left menus')
            count = count - 1
